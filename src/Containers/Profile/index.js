  ...
  const updateUser = async () => {
    ...
    if ((name || Address).length > 0) {
      ...
      const res = await axios
        .patch(`http://192.168.1.8:8080/users/updatewithpass`, {
          _id: user._id,
          name: name,
          address: Address,
          // mobile: Phone,
    ...
    if (Phone.length > 0) {
      setLoading(true)
      axios
        .patch(`http://192.168.1.8:8080/users/resetuniquemobile`, {
          _id: user._id,
          mobile: Phone,
        })
        .then(result => {
    ...
    if (Email.length > 0) {
      setLoading(true)
      axios
        .patch(`http://192.168.1.8:8080/users/resetuniqueemail`, {
          _id: user._id,
          email: Email,
        })
        .then(result => {
    ...
    if ((Password || Password2).length > 0) {
      ...
      if (Password === Password2) {
        setLoading(true)
        axios
          .patch(`http://192.168.1.8:8080/users/reset/password`, {
    ...
    if (photo === true) {
      ...
      setLoading(true)
      await axios
        .post(`http://192.168.1.8:8080/users/uploadprofile`, {
          mainform,
          config,
          // _id: user._id,
          // profile_pic: photo
